...
from tkinter import Tk, filedialog, messagebox, Text, Scrollbar, END, Frame, Canvas, Label, StringVar, ttk
...
def send_file_to_server(port, filename):
    try:
        ...
        store_transfer_log(filename, port)
...
def open_output_file():
    ...
    search_window = Tk()
    search_window.title("🔎 Search Reconstructed File")
    search_window.geometry("800x600")
    search_window.configure(bg="#ffffff")

    search_frame = Frame(search_window, bg="#ffffff")
    search_frame.pack(pady=5)

    search_var = StringVar()
    case_sensitive_var = tk.BooleanVar()

    tk.Label(search_frame, text="Search Word:", font=("Arial", 12), bg="#ffffff").pack(side="left", padx=5)
    search_entry = tk.Entry(search_frame, textvariable=search_var, font=("Arial", 12), width=30)
    search_entry.pack(side="left", padx=5)

    case_checkbox = tk.Checkbutton(search_frame, text="Case Sensitive", variable=case_sensitive_var, bg="#ffffff", font=("Arial", 10))
    case_checkbox.pack(side="left", padx=5)

    file_text = Text(search_window, wrap="word", font=("Courier New", 11))
    file_text.pack(fill="both", expand=True, padx=10, pady=10)

    scrollbar = Scrollbar(file_text)
    ...
    def search_word(*args):
        ...
        file_text.tag_remove("highlight", "1.0", END)
        ...
        idx = "1.0"
        while True:
            idx = file_text.search(word, idx, nocase=not case_sensitive_var.get(), stopindex=END)
            if not idx:
                break
            lastidx = f"{idx}+{len(word)}c"
            file_text.tag_add("highlight", idx, lastidx)
            idx = lastidx

        file_text.tag_config("highlight", background="yellow", foreground="black")
        file_text.config(state="disabled")

    search_var.trace_add("write", search_word)
...
console_input.bind("<Return>", lambda event: handle_console_input(event))
